...
const ChatSectionContainer = styled.div<{ isRequestsChat: boolean,scrollable:boolean }>`
  ...
  .chat-bubble-section {
    ...
    position: ${props => props.scrollable ? "relative" : "absolute"};
    right: 0;
    bottom: 0;
    width: 100%;
  }
...
const ChatSection = () => {
    ...
    // 채팅 메세지의 양에 따라서 UI 를 다르게 보여줍니다
    useEffect(()=>{

        if(chatMessageList.length < 11){
            sectionRef.current.scrollTop = sectionRef.current.scrollHeight
        }

        if(scrollCheckRef.current.offsetHeight + 5 > sectionRef.current.offsetHeight ){
            setScrollable(true)
        }else{
            setScrollable(false)
        }
   ...
   useEffect(()=>{
    ...
    }, [selectedRoom, chatMessageListPage]);
